...
function healthy_has_switched_users() {
	...
	if ( ! isset( $_COOKIE['healthy_active_user'] ) ) { return false; }

	// $active_user refers to the user whose been switched to, or, failiing that, the active user.
	$active_user_id = absint( $_COOKIE['healthy_active_user'] );
 ...
 * @param string Any role slug naem, such as teacher or student.
...
function healthy_user_is_role( $active_user = true, $role = 'teacher' ) {
	...
	// Tf there is a teacher role, return true.
...
/**
 * Determine if the current user has the authority to act on a post.
 *
...
function healthy_current_user_can_act_on_object( $object_id, $action, $object_type ) {
	...
	// Whitelist our pbject types.
	if( ( $object_type != 'post' ) && ( $object_type != 'user' ) && ( $object_type != 'week' ) ) { return false; }
...
function healthy_is_profile_complete() {
	...
	foreach( $user_fields as $field ) {
		...
		if( isset( $field[ 'is_hidden_from_teachers' ] ) && healthy_user_is_role( false, 'teacher' ) ) {
			continue;
		}
...
function healthy_is_week_full( $week = false ) {
	...
	$monday_in_seconds = strtotime( $monday );

	// Will get incremented as we loop through days.	
	$next_day_in_seconds = $monday_in_seconds;
	...
	while( $i < 7 ) {
		...
		// If we've looped past monday, calculate the day we're on in seconds
		if( $i > 1 ) {
			$next_day_in_seconds = $monday_in_seconds + ( ( DAY_IN_SECONDS * $i ) -1 );
		}
